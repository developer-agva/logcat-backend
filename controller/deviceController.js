...
const getWMYDataCountForAgvaPro = async (req, res) => {
  try {
    ...
    if (req.params.filter == "yearly") {
      ...
      // Helper function to get the date of the start of the week given a year and week number
      ...
      // Helper function to get the month and week number within the month from a date
      ...
      const groupByWeek = (data) => {
        return data.reduce((acc, item) => {
          ...
          const week = Math.ceil(((date - new Date(year, 0, 1)) / (24 * 60 * 60 * 1000) + new Date(year, 0, 1).getDay() + 1) / 7);
          const duration = `${year}-W${week}`;
          if (!acc[duration]) {
            acc[duration] = [];
          }
          acc[duration].push(item);
          return acc;
        }, {});
      };

      // Convert the weekly data to the desired format
      const convertWeeklyData = (dataCountByWeek) => {
        return dataCountByWeek.map(item => {
          const [year, week] = item.duration.split('-W');
          const date = getDateOfISOWeek(parseInt(year), parseInt(week));
          const { month, weekNumber } = getMonthAndWeek(date);
          const monthName = monthNames[month];
          return {
            duration: `${monthName}-W${weekNumber}`,
            count: item.count
          };
        });
      };

      // Helper function to filter data by a given time range
      const filterByTimeRange = (data, weeks) => {
        const targetDate = new Date();
        targetDate.setDate(targetDate.getDate() - 7 * weeks);
        return data.filter(item => item.createdAt >= targetDate);
      };
      ...
      // Main function to process data
        ...
        // Filter data to include only the last 12 weeks
        const last12WeeksData = filterByTimeRange(filteredProductionData, 12);

        // Group the matched entries by week for the last 12 weeks
        const groupedByWeekLast12Weeks = groupByWeek(last12WeeksData);

        // Count the entries per week for the last 12 weeks
        const dataCountByWeekLast12Weeks = Object.keys(groupedByWeekLast12Weeks).map(duration => ({
          duration,
          count: groupedByWeekLast12Weeks[duration].length
        }));

        // Convert the data to the desired format
        const totalDevicesCountWeekly = convertWeeklyData(dataCountByWeekLast12Weeks);

        // console.log('Data count by week for the last 12 weeks:', totalDevicesCountWeekly);
      // Call the function
      // processWeeklyData(req, res);
      return res.status(200).json({
        statusCode: 200,
        statusValue: "SUCCESS",
        message: "Data count retrieved successfully.",
        totalDevicesCountWeekly:totalDevicesCountWeekly.slice(-4),
      });
...
const getWMYDemoDataCountForAgvaPro = async (req, res) => {
  try {
    ...
    if (req.params.filter == "yearly") {
      ...
      const groupByWeek = (data) => {
        return data.reduce((acc, item) => {
          const duration = `${item.createdAt.getFullYear()}-W${Math.ceil((item.createdAt.getDate() - item.createdAt.getDay() + 1) / 7)}`;
          if (!acc[duration]) {
            acc[duration] = [];
          }
          acc[duration].push(item);
      ...
      // Function to get data count by week for the last 'n' weeks
      const getDataCountByWeek = (data, weeks) => {
        const filteredData = filterByTimeRange(data, weeks);
        const groupedByWeek = groupByWeek(filteredData);
        return Object.keys(groupedByWeek).map(duration => ({
          duration,
          count: groupedByWeek[duration].length
        }));
      };

      // Get data count by week for the last 4 weeks
      // const dataCountByWeekLast4Weeks = getDataCountByWeek(filteredProductionData, 4);

      return res.status(200).json({
        statusCode: 200,
        statusValue: "SUCCESS",
        message: "Data count retrieved successfully.",
        totalDevicesCountWeekly: dataCountByWeekLast4Weeks,
      });
      ...
      if (filter == "today") {
        ...
        return res.status(200).json({
          ...
          todayActiveDeviceCount:[
            ...
            // {
            //   duration:"7 H",
            //   count:results[0].eventsForLast1Hr.length,
            // },
        ...
        let result = await event_ventilator_collection.find({},{updatedAt:1,did:1}).limit(10);
        ...
        return res.status(200).json({
          ...
          const monthlyData = result.map(item => ({
            // year: item._id.year,
            duration: `${item._id.year}-${item._id.month}`,
            count: item.uniqueDidCount
          }));

          // Step : Function to convert date format
          const convertDateFormat = (monthlyData) => {
          ...
          const convertedData = convertDateFormat(monthlyData) 
          
          return res.status(200).json({
            statusCode: 200,
            statusValue: "SUCCESS",
            message: "Most recent events by device ID retrieved successfully.",
            monthlyDataCount: convertedData,
          });
          ...
          const pipeline = [
            {
                $match: {
                    date: { $gte: fiveYearsAgo.toISOString() }
                }
            },
            {
                $addFields: {
...
const getActiveDemoDevicesCountForAgvaPro = async (req, res) => {
  ...
  if (filter == "yearly") {
    ...
    // Step 1: Extract the year from each event's eventDate and group by year
      const yearlyData = eventData.reduce((acc, event) => {
        const year = new Date(event.eventDate).getFullYear();
        if (!acc[year]) {
            acc[year] = new Set();
        }
        acc[year].add(event.did);
        return acc;
      }, {});

      // Step 2: Convert the grouped data into the desired output format
      const yearlyDataCount = Object.keys(yearlyData).map(year => ({
        year: parseInt(year),
        duration: parseInt(year),
        count: yearlyData[year].size
      }));
      ...
      return res.status(200).json({
        ...
        statusValue: "SUCCESS",
        message: "Data count retrieved successfully.",
        yearlyDataCount:yearlyDataCount
      })

  }  else if (filter == "monthly") {
  
    // Step 1: Extract the month and year from each event's eventDate and group by month and year
    ...
    const monthlyData = eventData.reduce((acc, event) => {
        ...
        acc[key].add(event.did);
        return acc;
    }, {});

    // Step 2: Ensure the data covers the last 12 months

    const monthlyDataCount = months.map(({ year, month }) => {
    const key = `${year}-${month}`;
      return {
        // year:year,
        duration:key,
        count: monthlyData[key] ? monthlyData[key].size : 0
      };
    });

    // Step 3: Sort the data in ascending order by year and month
    // monthlyDataCount.sort((a, b) => (a.year === b.year ? a.month - b.month : a.year - b.year));
    monthlyDataCount.sort((a, b) => {
      if (a.year === b.year) {
    ...
    const convertDateFormat = (monthlyDataCount) => {
      return monthlyDataCount.map(item => {
        const [year, month] = item.duration.split('-');
        const monthName = monthNames[parseInt(month)-1];
        return {
          ...item,
    ...
    const convertedData = convertDateFormat(monthlyDataCount);
    ...
    return res.status(200).json({
      ...
      statusValue: "SUCCESS",
      message: "Data count retrieved successfully.",
      monthlyDataCount:convertedData.reverse()
    })

  } else if (filter == "weekly") {
    // Step 1: Determine the date range for the last 4 weeks
    ...
    for (let i = 0; i < 4; i++) {
        ...
        startOfPreviousWeeks.push(startOfWeek);
    }

    // Step 2: Group events by week and count unique device IDs
    const weeklyData = eventData.reduce((acc, event) => {
      const eventDate = new Date(event.eventDate);
      const eventWeekStart = getStartOfWeek(new Date(eventDate));
      
      startOfPreviousWeeks.forEach((startOfWeek, index) => {
          if (eventWeekStart.getTime() === startOfWeek.getTime()) {
              if (!acc[index]) {
                  acc[index] = new Set();
              }
              acc[index].add(event.did);
          }
      });
    ...
    // Step 3: Convert the grouped data into the desired output format
    const weeklyDataCount = startOfPreviousWeeks.map((startOfWeek, index) => ({
      duration: startOfWeek.toISOString().split('T')[0], // Get date string in YYYY-MM-DD format
      count: (weeklyData[index] ? weeklyData[index].size : 0)
    }));
    ...
    const convertWeeklyData = (weeklyDataCount) => {
      return weeklyDataCount.map(item => {
    ...
    // Convert the data
    const convertedData = convertWeeklyData(weeklyDataCount);

    return res.status(200).json({
      statusCode: 200,
      statusValue: "SUCCESS",
      message: "Data count retrieved successfully.",
      weeklyDataCount:convertedData.reverse()
    })    
