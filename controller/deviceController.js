...
/**
 * api      PUT @/devices/update-payment-lock-status/:DeviceId
 * desc     @update paymentDoneInPercent for device
 */
const updateDevicePaymentAndLockStatus = async (req, res) => {
  try {
    const { DeviceId } = req.params;
    ...
    return res.status(200).json({
      statusCode: 200,
      statusValue: "SUCCESS",
      message: "Device payment percentage updated successfully!",
      data: updatedDevice,
    });
...
const getDeviceOverviewForSalesById = async (req, res) => {
  try {
    ...
    data = {
      ...
      'isPaymentDone': !!(data.isPaymentDone) ? data.isPaymentDone : "true",
      'paymentDoneInPercent': !!(data.paymentDoneInPercent) ? data.paymentDoneInPercent : "0",
      'isLocked': !!(data.isLocked) ? data.isLocked : false,
      'lockedStatus': !!(data.lockedStatus) ? data.lockedStatus : "",
    };
    if (!data) {
...
/**
 * api      GET @/devices/get-payment-status/:DeviceId
 * desc     @get payment status for device overview button
 */
const getPaymentStatus = async (req, res) => {
  try {
    ...
    const paymentPercent = device.paymentDoneInPercent ? parseInt(device.paymentDoneInPercent) : 0;
    const isCompleted = paymentPercent === 100;
    const isPending = paymentPercent === 0;
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        paymentDoneInPercent: paymentPercent.toString(),
        isPaymentDone: device.isPaymentDone || "true",
        status: isCompleted ? "completed" : isPending ? "pending" : "partial",
        color: isCompleted ? "green" : isPending ? "red" : "yellow"
      }
    });
  } catch (err) {
    return res.status(500).json({
    ...
    const agvaProData = await productionModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          inHouseCount: { $sum: { $cond: { if: { $eq: ["$purpose", "In-House"] }, then: 1, else: 0 } } }
        }
      },
      {
        $project: {
...
const getWMYDataCountForAgvaPro = async (req, res) => {
  try {
    ...
    if (req.params.filter == "yearly") {
      ...
      const recentProductionData2 = productionData.filter(production => production.createdAt >= twelveMonthsAgo);
...
module.exports = {
  ...
  updateDevicePayment,
  updateDevicePaymentAndLockStatus
}